                ...
                if let Some(seat) = maybe_seat {
                    ...
                    if let Some(ref mut gesture_state) = self.common.gesture_state {
                        ...
                        if first_update {
                            ...
                            activate_action = self.common.config.gestures.0.iter().find(|(gesture, action)| {
                                gesture.fingers == gesture_state.fingers && Some(gesture.direction) == gesture_state.direction
                            }).map(|(_, action)| action).cloned();
                            gesture_state.action = activate_action.clone();
                        }

                        match gesture_state.action {
                            Some(shortcuts::Action::NextWorkspace) | Some(shortcuts::Action::PreviousWorkspace) => {
